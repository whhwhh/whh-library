...
<table width="776"  border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
	...
	<?php
		...
		$sql=mysqli_query($conn,"select r.*,t.name as typename,t.number from tb_reader r left join tb_readerType t on r.typeid=t.id where r.barcode='$barcode'");
                  ...
                  <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
                        ...
                        <table width="100%" height="21" border="0" cellpadding="0" cellspacing="0">
                          <tr>
                            ...
                            <td width="76%" style="padding-top:7px;">读者条形码：
                              <input name="barcode" type="text" id="barcode" size="24" <?php if($_POST!=NULL){?>value="<?php echo $info['barcode'];?>" <?php }?>>
                            &nbsp;
                    ...
                    <tr>
                      <td align="center"><table width="96%" border="0" cellpadding="0" cellspacing="0">
                          <tr>
                           <td height="27">姓&nbsp;&nbsp;&nbsp;&nbsp;名：
                       <input name="readername" type="text" id="readername" <?php if($_POST!=NULL){?>value="<?php echo $info['name'];?>" <?php }?>></td>
                 <td>性&nbsp;&nbsp;&nbsp;&nbsp;别：
                   <input name="sex" type="text" id="sex" <?php if($_POST!=NULL){?>value="<?php echo $info['sex'];?>"<?php }?>></td>
               </tr>
               <tr>
                 <td height="27">证件类型：
                   <input name="paperType" type="text" id="paperType" <?php if($_POST!=NULL){?>value="<?php echo $info['paperType'];?>"<?php }?>></td>
                 <td>证件号码：
                   <input name="paperNo" type="text" id="paperNo" <?php if($_POST!=NULL){?>value="<?php echo $info['paperNO'];?>"<?php }?>></td>
               </tr>
               <tr>
                 <td height="27">读者类型：
                   <input name="readerType" type="text" id="readerType" <?php if($_POST!=NULL){?>value="<?php echo $info['typename'];?>" <?php }?>></td>
                 <td>可借数量：
                   <input name="number" type="text" id="number" <?php if($_POST!=NULL){?>value="<?php echo $info['number'];?>" <?php }?> size="17">
                   册
                   &nbsp;</td>
                          </tr>
                      </table></td>
                 ...
                 <tr>
                   <td height="32">&nbsp;添加的依据：
                     ...
                     <input name="operator" type="hidden" id="operator" value="<?php echo $_SESSION['admin_name'];?>">
					  <input name="Button2" type="button" class="btn_grey" id="Button2" onClick="window.location.href='bookBorrow.php'" value="完成借阅">                   
					  </td>
                 </tr> 
            <tr>
...
$query=mysqli_query($conn,"select * from tb_bookinfo where $f='$inputkey'");
$result=mysqli_fetch_array($query);   //检索图书信息是否存在
if($result==false){
	echo "<script language='javascript'>alert('该图书不存在！');window.location.href='bookBorrow.php?barcode=$barcode';</script>";
}
else{
	$query1=mysqli_query($conn,"select r.*,borr.borrowTime,borr.backTime,book.bookname,book.price,pub.pubname,bc.name as bookcase from tb_borrow as borr join tb_reader as r on borr.readerid=r.id join tb_bookinfo as book on book.id=borr.bookid join tb_publishing as pub on book.ISBN=pub.ISBN  join tb_bookcase as bc on book.bookcase=bc.id  where borr.bookid=$result[id] and borr.readerid=$readerid and ifback=0");   //检索该读者所借阅的图书是否与再借图书重复
	...
	if($result1==true){    //如果借阅的图书已被该读者借阅，那么提示不能重复借阅 
		echo "<script language='javascript'>alert('该图书已经借阅！');window.location.href='bookBorrow.php?barcode=$barcode';</script>";
			...
			$bookid=$result['id'];
			mysqli_query($conn,"insert into tb_borrow(readerid,bookid,borrowTime,backTime,operator,ifback)values('$readerid','$bookid','$borrowTime','$backTime','$_SESSION[admin_name]',0)");
