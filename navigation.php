<?php
if(!session_id()) include ("check_login.php"); 
include("conn/conn.php");
$n=$_SESSION['admin_name'];
$query=mysqli_query($conn,"select m.id,m.name,p.id,p.sysset,p.readerset,p.bookset,p.borrowback,p.sysquery from tb_manager as m left join (select * from tb_purview ) as p on m.id=p.id where name='$n'");
